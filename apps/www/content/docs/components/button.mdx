...
Use the `accessibleWhenDisabled` prop to keep a disabled button in the keyboard
focus order. This is useful when discoverability is important, such as toolbar
actions that are temporarily unavailable based on the current selection or
clipboard contents.

For buttons that enter a loading state after being clicked, this also keeps
focus on the button while it is disabled and preserves the tab order.

<ExampleTabs name="button-with-accessible-when-disabled" />
